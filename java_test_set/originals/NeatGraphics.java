...
public class NeatGraphics extends Applet implements MouseListener{
   ...
   public void paint (Graphics g){
      //winW and winH store the width and height of the window, so that the balls bounce off the window no matter what size
      squad.draw(gBuffer);
...
class Circle{
   ...
   public Circle(int xParam, int yParam){
      ...
      //at the moment it makes sure neither of them is zero, so the balls never travel in a horizontal or vertical line
      while(i == 1){
   ...
   //simply reverses the velocity in either the x or y direction depending on which wall it collides with
   ...
   public void checkBallCollision(Circle circle2){
      ...
      //If they overlap do all these calculations I found online to see what direction they keep traveling in
      //This does take into account the mass of the balls. A larger ball will be less affected by a smaller ball hitting it.
      if(distance <= this.rad + circle2.rad){
         int newXVel1 = (this.xVel * (this.rad - circle2.rad) + (2*circle2.rad * circle2.xVel)) / (this.rad + circle2.rad);
         int newYVel1 = (this.yVel * (this.rad - circle2.rad) + (2*circle2.rad * circle2.xVel)) / (this.rad + circle2.rad);
         int newXVel2 = (circle2.xVel * (circle2.rad - this.rad) + (2* this.rad * this.xVel)) / (this.rad + circle2.rad);
         int newYVel2 = (circle2.yVel * (circle2.rad - this.rad) + (2* this.rad * this.yVel)) / (this.rad + circle2.rad);
         this.xVel = newXVel1;
         this.yVel = newYVel1;
